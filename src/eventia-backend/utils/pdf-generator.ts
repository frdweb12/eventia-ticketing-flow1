...
import { Booking, DeliveryDetails, Event } from '../models';
...
export const generateTicketPDF = (
  ...
  doc.text(`Booking Date: ${new Date(booking.booking_date).toLocaleDateString()}`, 20, 92);
  doc.text(`Number of Tickets: ${booking.seats.length}`, 20, 99);
